...
class ComModel extends CI_Model
{
  ...
  {
    $this->db->select('r.*,ind.industeries_name as ind,bus.business_type');
    ...
    public function showAllDaLaTen($data,$con,$table) {
        return $this->db->select($data)->where($con)->order_by('id', 'desc')->limit(10)->get($table)->result_array();
    ...
    public function disData($data,$con,$table)
    {
        return $this->db->select($data)->where($con)->get($table)->row_array();

    }
    public function showData($data,$con,$table)
    {
        return $this->db->select($data)->where($con)->get($table)->row_array();
    }
    ...
    public function update_data($data,$table) {
     ...
     public function update_data_1($data,$table) {
     ...
     {
        $this->db->select('b.business_type,s.sector_name,s.description');
        $this->db->join('sector as s','s.id=b.sector_name');
        $query = $this->db->get('business as b');
        return $query->result_array();
