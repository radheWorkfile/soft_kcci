...
class ComModel extends CI_Model
{
    ...
    public function getLastfouId()
    {
        return $this->db->select('id')->order_by('id', 'desc')->limit(1)->get('founder')->row_array();
    }
    public function getLastMenId()
    {
        return $this->db->select('id')->order_by('id', 'desc')->limit(1)->get('mentor')->row_array();
    }
	...
	function mi_update_data($table, $con, $data)
	{
	    $this->db->where($con);
